...
| 역할 | 경로 |
|------|------|
| 페이지 본문 + `<template>` 프리로드 | `_pages/labs/ielts/index.md` |
| 레이아웃 (`body.ielts-lab-page`, include 연결) | `_layouts/labs-ielts.html` |
| 탑바(브레드크럼 콤보, READER, Passage, 하이라이트 CLEAR) | `_includes/preview/sidebar-ielts.html` |
| IELTS 전용 CSS | `_includes/preview/ielts-lab-styles.html` |
| IELTS 전용 JS | `_includes/preview/ielts-lab-scripts.html` |
...
- **`labs-ielts` 레이아웃**: `preview/styles.html`, `preview/scripts.html` + IELTS 전용 include.
...
- **콤보 스타일**: 화살표 숨김, 테두리 0, 모노 느낌, 뮤트 컬러 등 (`ielts-lab-styles.html` 내 `.ielts-lab-page .sidebar select.combo` 오버라이드).
- **라벨 대문자** 표기.
