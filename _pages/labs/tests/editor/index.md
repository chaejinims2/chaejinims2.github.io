...
<div class="voca-editor">
  ...
  {% assign properties = site.data[page.dataset][page.data].properties %}
      ...
      <thead>
        <tr>
          {% for property in properties %}
            <th class="col-{{ property.property_id }}">{{ property.label }}</th>
          {% endfor %}
        </tr>
      </thead>
...
<script
  ...
  id="ve-words-data"
  data-words-src="{{ '/assets/data/' | append: page.dataset | append: '/words.json' | relative_url }}"
></script>

<script src="{{ '/assets/js/pages/voca-editor.js' | relative_url }}"></script>
<script>
  // PJAX/직접 진입 모두 대응: 스크립트가 로드된 상태면 바로 초기화
  try { window.initVocaEditorPage && window.initVocaEditorPage(); } catch (e) { }
</script>