 ...
 *  */ 
  ...
  function exportJson() {
    ...
    // words.json 원본 형태(최상위 배열)로 내보낸다.
    const blob = new Blob([JSON.stringify(normalized.words, null, 2)], { type: "application/json;charset=utf-8" });
  ...
  function bindUI() {
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", async () => {
        const normalized = ensureSchema(model);
        const txt = JSON.stringify(normalized.words, null, 2);
        try {
