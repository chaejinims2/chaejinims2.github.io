...
(function () {
  ...
  function main() {
    ...
    function syncDirLabel(dirEl) {
      var label = document.querySelector('[data-dir-label]');
    ...
    function applyScale() {
      var sel = document.querySelector('input[name="scale"]:checked');
    ...
    document.querySelectorAll('input[name="scale"]').forEach(function (r) { r.addEventListener('change', applyScale); });
    applyScale();
    ...
    function getLayerIndex(key) {
      var inp = layerHost.querySelector('[data-layer-index="' + key + '"]') || document.querySelector('[data-layer-index="' + key + '"]');
      if (!inp) return 0;
    ...
    function getLayerDst(key) {
      ...
      var ix = layerHost.querySelector('[data-layer-dx="' + key + '"]') || document.querySelector('[data-layer-dx="' + key + '"]');
      var iy = layerHost.querySelector('[data-layer-dy="' + key + '"]') || document.querySelector('[data-layer-dy="' + key + '"]');
    ...
    function syncPantsOffsetFromSpec(ps) {
      ...
      var ix = document.querySelector('[data-layer-dx="pants"]');
      var iy = document.querySelector('[data-layer-dy="pants"]');
    ...
    function syncPickerOutLabels() {
      ['shirt', 'acc', 'hair', 'hat'].forEach(function (k) {
        var out = document.querySelector('[data-picker-out="' + k + '"]');
    ...
    function wireUi() {
      ...
      syncDirLabel(dirSel);
      document.querySelectorAll('[data-dir-delta]').forEach(function (btn) {
        btn.addEventListener('click', function () {
          if (!dirSel) return;
    ...
    fetch('./layout.json')
      .then(function (r) { if (!r.ok) throw new Error('layout.json HTTP ' + r.status); return r.json(); })
      .then(function (data) {
