...
Purpose
- Similar in concept to Air Traffic Control, but for satellites.
- Aim: avoid collisions; needs legal measures and technical [Q31]

Why STM is difficult
- Satellites are relatively [Q32] now → more launches.
- Operators launch whole [Q33] (thousands of linked satellites).
- A satellite in space does not have to send [Q34] to Earth.
- Not enough reliable ways of [Q35] satellites.

Other issues
- Space debris (some pieces very small).
- Some operators unwilling to share information (e.g. [Q36] or commercial reasons).
- Hard to collect data on the object and its [Q37] at a given time.
- Scientists can only make a [Q38] about where a satellite will go next.

Possible solutions
- Shared standards for how information is presented.
- A single [Q39] for all the data.
- A system that coordinates operators and builds [Q40] (nationally and globally).
```
